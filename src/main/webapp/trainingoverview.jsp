                    ...
                    <c:forEach var="trainingen" items="${allTrainings}">
                        ...
                        <table id="${trainingen.key.userid}" }>
                            ...
                            <tr>
                                ...
                                <th class="TOtableHeading" onclick="sortTableDate(${trainingen.key.userid}, 1)">Date
                                </th>
                                <th class="TOtableHeading" onclick="sortTableTime(${trainingen.key.userid}, 2)">Start
                                </th>
                                <th class="TOtableHeading" onclick="sortTableTime(${trainingen.key.userid}, 3)">End</th>
                                <th class="TOtableHeading" onclick="sortTableNumber(${trainingen.key.userid}, 4)">
                                    Duration (in min)
                                </th>
                                <th>Edit</th>
                                <th>Delete</th>
                            ...
                            <c:forEach var="training" items="${trainingen.value}" varStatus="status">
                                <tr>
                                    ...
                                    <c:if test="${sessionScope.user.userid eq training.userID || sessionScope.user.role eq 'ADMIN'}">
                                        <td><a id="edit"
                                               href="Controller?command=ToEditTraining&trainingid=${training.trainingId}">Edit</a>
                                        </td>
                                        <td><a id="delete"
                                               href="Controller?command=DeleteTraining&trainingid=${training.trainingId}">Delete</a>
                                        </td>
                                    </c:if>
